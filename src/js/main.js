var Weather = {
	...
	setBackground: function(value) {
		...
		$("html").css({background: 'url('+ Weather.backgroundArray[value] +') no-repeat fixed center center / cover'});
	},
	...
	insertMainCard: function(response) {
		...
		var weatherText = response.weather[0].description.replace(/\b\w/g, function(match) { return match.toUpperCase(); });
	...
	toggleAllUnits: function() {
		...
		$.each($values, function(i, item) {
			...
			item.innerHTML = Weather.helper.
				toggleUnit(Weather.helper.
					clearValueImput(item.innerHTML));
